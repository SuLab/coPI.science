...
| `prompts/email-reply-classify.md` | *Currently unused by any code — `classify_reply` in `src/services/email_inbound.py` builds its classification prompt inline instead. Kept on disk, not deleted, in case a future refactor wants to externalize the prompt again (#26 DOC-5).* |
