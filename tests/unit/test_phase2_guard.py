...
pitch per day (`lab_daily_post_cap`), while other roles (e.g. `scout_hub`)
keep the pre-existing `daily_post_cap`.
"""
import inspect
import types
...
async def test_scout_hub_with_one_post_still_reaches_llm(monkeypatch):
    """scout_hub is not subject to lab_daily_post_cap — daily_post_cap=5
    still has headroom at 1 post, so the turn must proceed to the LLM."""
