"""AGENT.md factual corrections: stale GitHub URL, wrong impersonation route,
and email listed as out of scope though fully built.
"""
...
def test_impersonation_path_matches_the_real_route():
    ...
    # Anchored to the exact route form: a bare substring check is vacuous
    # because "/api/admin/impersonate" also contains "/admin/impersonate".
...
def test_daily_digest_is_not_listed_as_out_of_scope_but_is_documented_as_built():
    """The periodic status digest (check_and_send_status_overviews, wired daily
    in src/worker/main.py) must not be listed under "What's Out of Scope" while
    the adjacent built-features paragraph documents it as shipped —
