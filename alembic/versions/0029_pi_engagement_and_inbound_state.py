...
Two nullable markers. Both are nullable with no server default and no backfill:
NULL means "unknown", and every reader must treat unknown as the pre-0029
behaviour.

thread_decisions.pi_engaged_at
-------------------------------
...
Making ProposalReview.user_id (src/models/agent_registry.py) nullable instead
was rejected: it is ForeignKey("users.id", ondelete="CASCADE"), so deleting a
PI would cascade-delete the engine's own block-clearing markers. A timestamp
on the thread's own decision row has no CASCADE exposure to users at all
(thread_decisions cascades from simulation_runs only).
...
agent_messages.pi_inbound_state
---------------------------------
...
ACCESS EXCLUSIVE lock, regardless of agent_messages' size.
