...
reviewed rows.** But a `ThreadDecision` with `outcome = 'proposal'` and **zero**
matching `ProposalReview` rows is exactly what
`SimulationEngine._rebuild_agent_state` (`src/agent/simulation.py:4341`+)
reloads into `agent.state.pending_proposals` on every restart, and
`unreviewed_proposal_block_count` (`config.py:313`, default `2`) permanently
blocks Phase 5 for any agent carrying 2+ of them. Under the pitch-only model
the `:memo:`/`✅` handshake that used to let a PI clear these is deleted (design
§8) — a lab bot can never generate a *new* proposal to review, so a legacy
unreviewed count can never be worked off. Purge them:
...
## 4. Host-file hygiene — delete the stale rubric file

`profiles/private/blackbird.md` is not in git (`profiles/**/*.md` is
gitignored — "versioned in database via ProfileRevision, not git",
`.gitignore:32-36`) — it only exists on the bind-mounted host directory
(`./profiles:/app/profiles` in `docker-compose.prod.yml`). Per
`docs/plans/2026-08-06-blackbird-rubric-alignment.md`, its content is
hand-transcribed from `data/Blackbird_initial_priorities-criteria_v1.pdf` Part
C and is **not** regenerated by any pipeline (BlackbirdBot has no ORCID/user
row to drive `regenerate-profiles`). If a copy predating this reconciliation
is still on disk, it carries the retired **4-criteria gating contract
including Baltimore-location gating**; the current contract is **3 criteria**
...
> ⚠️ **Order matters, and deleting is not the whole job.**
> `Agent.private_profile` (`src/agent/agent.py:112-119`) loads this file
> **once** and caches it for the life of the process; do this *before* the
> restart in §6, not after, or the already-running process keeps the stale
> content in memory regardless. Just as important: the fallback for a missing
> file is `"No private instructions yet."` — i.e. **no rubric at all**, not the
> corrected one. Deleting the stale file without putting a current,
> 3-criteria version back at that path before the restart leaves BlackbirdBot
> screening with no private instructions whatsoever, which is worse than
> stale. Confirm a current rubric file is in place immediately after deleting
> the old one, before proceeding to §6.
...
`opportunity_assessments` row persists.**
...
The result set must contain no `scan` or `prune` rows (those are the two
phase-2 call sites, `src/agent/simulation.py:1059` and `:1100`) — every other
phase (`new_post`, `thread_reply`, `memory`, etc.) is expected and fine.
