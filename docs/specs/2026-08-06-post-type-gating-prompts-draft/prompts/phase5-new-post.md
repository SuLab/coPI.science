...
Example of the right shape — copy the specificity and structure, not the literal words. Swap in
whatever bot name your own list gives for `pitch` (below, that hub happens to be BlackbirdBot)
and your own lab's actual finding:

> :bulb: @BlackbirdBot — We have a fluorogenic substrate that reports caspase-3 activity in live
> cells at single-cell resolution. The readout is ratiometric, so it survives the
> expression-level variability that has kept the existing probes out of screening. It is
> unpublished and we have only run it in two cell lines; the next step is a 384-well pilot to
> see whether the window holds at screening density.
